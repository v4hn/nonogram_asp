...
echo "done"
echo
l=0; while read line; do [[ "$line" = "done" ]] && break; x=0; for i in $line; do echo "row($l,$x,$i)."; let x++; done; let l++; done > nonogram_tmp.lp
echo "row_cnt($l)." >> nonogram_tmp.lp
echo "now enter column blocks:"
l=0; while read line; do [[ "$line" = "done" ]] && break; x=0; for i in $line; do echo "col($l,$x,$i)."; let x++; done; let l++; done >> nonogram_tmp.lp
